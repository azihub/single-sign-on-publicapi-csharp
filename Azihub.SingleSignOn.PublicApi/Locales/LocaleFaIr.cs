...
{
    /// <summary>
    /// Class container of Persian translation of localization.
    /// </summary>
    public static class LocaleFaIr
    {
        ...
        /// <summary>
        /// This method get all LocaleConsts and LocaleFaIr class fields by reflections.
        /// It then checks that all LocaleConsts class fields are available in LocaleFaIr.
        ...
        {
            // Get all field by reflection in LocaleConsts and LocaleFaIr
            var localizationField = typeof(LocaleConsts).GetFields(BindingFlags.Public | BindingFlags.Static);
            var localeFaIr = typeof(LocaleFaIr).GetFields(BindingFlags.Public | BindingFlags.Static);

            // Define a dictionary to add field names and values
            var data = new Dictionary<string, string>();
            foreach (var item in localizationField)
            {
                // Get field value from LocaleFaIr
                var getLocaleFaIrField = typeof(LocaleFaIr).GetField(item.Name);

                // Check field in localeFaIr
                var checkLocaleFaIrFeild = localeFaIr.Any(x => x.Name == item.Name);
                if (checkLocaleFaIrFeild == false)
                    throw new NullReferenceException();

                // Add field to data Dictionary
                data.Add($"{item.Name}", $"{getLocaleFaIrField.GetValue(item.Name)}");
            }
